...
export default function Cadastro() {
  ...
  const [confirmaSenhaError, setConfirmaSenhaError] = useState(null);
  ...
  const compararSenhas = (senha, confirmaSenha) => senha === confirmaSenha;

  const handleEntrarPress = () => {
    const isEmailValid = validarEmail(email);
    const isSenhaValid = validarSenha(senha);
    const areSenhasIguais = compararSenhas(senha, confirmaSenha);

    setEmailError(isEmailValid ? null : "Email inválido");
    setSenhaError(
      isSenhaValid ? null : "Senha inválida. Deve ter pelo menos 6 caracteres."
    );
    setConfirmaSenhaError(areSenhasIguais ? null : "As senhas não coincidem");

    if (isEmailValid && isSenhaValid && areSenhasIguais) {
      ...
      console.log(
        "Dados inválidos. Verifique o email, a senha e a confirmação de senha."
      );
    }
  };
